...
var errorLog=[];var RUNTIME_COUNTER_Testing=0;var checkInProgress=function(){return RUNTIME_COUNTER_Testing>0;};var getType=function(value){var stringType;if(value===undefined||value===null){return"undefined";}
stringType=Object.prototype.toString.call(value);stringType=stringType.split(" ")[1];stringType=stringType.substring(0,stringType.length-1).toLowerCase();return stringType;};var createDebugObjectDisplay=function(target){var display=Object.create(null);var proto=Object.getPrototypeOf(target);for(var name in target){if(target.hasOwnProperty(name)){display[name]=target[name];}}
...
throw new Error("types.js -[private] assertType(name,target,checker)");};types.isNull=function(value){return getType(value)==="undefined";};types.isNotNull=function(value){return getType(value)!=="undefined";};types.isAny=function(value){return true;};types.isBoolean=function(value){return getType(value)==="boolean";};types.isNumber=function(value){var result=getType(value);return result==="number"?String(value)!=="NaN":false;};types.isString=function(value){return getType(value)==="string";};types.isObject=function(value){return getType(value)==="object";};types.isArray=function(value){return getType(value)==="array";};types.isFunction=function(value){return getType(value)==="function";};types.isPrototypeProperty=function(target,name){if(types.isNull(target)){throw new Error("types.isPrototypeProperty(target:Object,name:String): ERROR -> target is null.");}
